...
public:
	...
	virtual void Advection(real dt)
	{
		...
		for(int i=0;i<node_num;i++){
			VectorD pos_to_peek = Pos(i) - dt * u_copy[i] / 2;
			u[i] = Interpolate(u_copy, pos_to_peek);
		}
	}
	...
	virtual void Projection()
	{
		...
		for(int iter=0;iter<40;iter++){
			for(int i=0;i<node_num;i++){
				...
				VectorDi node=Coord(i);
				real sum = 0;
				auto i_mat = Matrix<real, d>::Identity();
				for (int dim = 0; dim < d; dim++)
				{
					sum += p[Idx(node + VectorDi::Unit(dim))] + p[Idx(node - VectorDi::Unit(dim))];
				}
				p[i] = (-div_u[i]*dx2 + sum) / (2 * d);
			}
		}
		...
		for(int i=0;i<node_num;i++){
			...
			for (int dim = 0; dim < d; dim++)
			{
				grad_p[dim] = p[Idx(node + VectorDi::Unit(dim))] - p[Idx(node - VectorDi::Unit(dim))];
			}
			u[i] -= grad_p / (2 * dx);
		}
	}
	...
	void Vorticity_Confinement(const real dt)
	{
		...
		for(int i=0;i<node_num;i++){
			...
			vor[i] = (u[Idx(node + Vector2i::UnitX())][1] - u[Idx(node - Vector2i::UnitX())][1]
				+ u[Idx(node + Vector2i::UnitY())][0] - u[Idx(node - Vector2i::UnitY())][0]) / (2 * dx);
		}
		...
		for(int i=0;i<node_num;i++){
			...
			N[i][0] = (vor[Idx(node + Vector2i::UnitX())] - vor[Idx(node - Vector2i::UnitX())]) / (2 * dx);
			N[i][1] = (vor[Idx(node + Vector2i::UnitY())] - vor[Idx(node - Vector2i::UnitY())]) / (2 * dx);
			N[i].normalize();
		}
